...
// --- Tiempo real (runRealtimeReport: usuarios activos en los últimos 30 min) ---

const limpiaTitulo = name =>
  String(name || '').replace(/\s*\|\s*Brisa(?:Plus|\s*\+)\s*$/i, '').trim();
...
export function realtimeRequests() {
  ...
  return {
    total: { metrics: [{ name: 'activeUsers' }] }, // sin dimensión: cuenta de-duplicada
    paginas: top('unifiedScreenName', 20),         // de sobra para filtrar sistema y quedarnos con 6
    paises: top('country', 5),
  };
}

export function normalizeRealtime({ total, paginas, paises } = {}) {
  const activos = NUM(total?.rows?.[0]?.metricValues?.[0]?.value);
  const pgs = (paginas?.rows || [])
    .map(r => ({ raw: r.dimensionValues[0].value, value: NUM(r.metricValues[0].value) }))
    .filter(p => !isSystemPage(p.raw))
    .slice(0, 6)
    .map(p => ({ label: limpiaTitulo(p.raw), value: p.value }));
  const pss = (paises?.rows || [])
    .map(r => ({ label: r.dimensionValues[0].value, value: NUM(r.metricValues[0].value) }));
  return { activos, paginas: pgs, paises: pss };
}
